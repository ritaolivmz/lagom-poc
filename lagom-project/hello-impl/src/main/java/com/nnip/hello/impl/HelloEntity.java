...
import com.nnip.hello.impl.HelloCommand.Hello;
import com.nnip.hello.impl.HelloCommand.UseGreetingMessage;
import com.nnip.hello.impl.HelloEvent.GreetingMessageChanged;
...
/*    b.setCommandHandler(UseGreetingMessage.class, (cmd, ctx) ->
    ...
    //evt -> new GreetingMessageChanged("yeah"));
    b.setReadOnlyCommandHandler(UseGreetingMessage.class,
            // Get the greeting from the current state, and prepend it to the name
            // that we're sending
            // a greeting to, and reply with that message.
            (cmd, ctx) -> ctx.reply(state().message + ", MY DEAR " + cmd.message + "!"));
    ...
    /*b.setReadOnlyCommandHandler(HelloCommand.Test.class,
            // Get the greeting from the current state, and prepend it to the name
            // that we're sending
            // a greeting to, and reply with that message.
            (cmd, ctx) -> ctx.reply(state().message + ", " + cmd.name + "!"));*/

    b.setCommandHandler(HelloCommand.Test.class, (cmd, ctx) ->
            // In response to this command, we want to first persist it as a
            // GreetingMessageChanged event
            ctx.thenPersist(new GreetingMessageChanged(cmd.message),
                    // Then once the event is successfully persisted, we respond with done.
                    evt -> ctx.reply(state().message + ", " + cmd.name + "!")));
